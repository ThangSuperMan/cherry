module github.com/cherryramatisdev/cherry

go 1.18

require github.com/rwxrob/bonzai v0.2.0

require (
	github.com/rogpeppe/go-internal v1.8.1 // indirect
	github.com/rwxrob/config v0.3.6 // indirect
	github.com/rwxrob/fn v0.3.3 // indirect
	github.com/rwxrob/fs v0.5.0 // indirect
	github.com/rwxrob/json v0.6.0 // indirect
	github.com/rwxrob/scan v0.6.1 // indirect
	github.com/rwxrob/structs v0.5.1 // indirect
	github.com/rwxrob/term v0.2.6 // indirect
	github.com/rwxrob/to v0.4.5 // indirect
	github.com/rwxrob/y2j v0.3.4 // indirect
	golang.org/x/crypto v0.0.0-20220331220935-ae2d96664a29 // indirect
	golang.org/x/mod v0.5.1 // indirect
	golang.org/x/sys v0.0.0-20220406163625-3f8b81556e12 // indirect
	golang.org/x/term v0.0.0-20210927222741-03fcf44c2211 // indirect
	gopkg.in/yaml.v3 v3.0.0-20210107192922-496545a6307b // indirect
)
